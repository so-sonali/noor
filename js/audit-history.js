...
function wireExperimentCapture(){
  ...
  document.addEventListener('submit',ev=>{if(ev.target?.id!=='experimentForm')return;const idInput=document.querySelector('#expId');if(!idInput.value)idInput.value=uid('exp');const id=idInput.value;(async()=>{const before=editBaselines.get(id)||await get('experiments',id);const previousUpdatedAt=before?.updatedAt;const after=await waitForSave(id,previousUpdatedAt);await recordRevision(id,before,after);})().catch(err=>console.error('NOOR revision history:',err));},true);
}
